...
(function () {
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Familia + articolele + Berufe + mein/dein</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — Articolul hotărât (der/die/das) pentru membrii familiei', 'der (m): Vater, Bruder, Sohn · die (f): Mutter, Schwester, Tochter · das (n): Kind. das Mädchen = neutru (capcană!).', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — Articolul nehotărât (ein/eine)', 'ein (m, n) · eine (f). Folosit pentru a introduce ceva ne-specific.', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillInBlock('Übung 3 — Negația „kein" (nu este / nu am)', 'kein (m, n) · keine (f). „Das ist kein Buch." = Asta nu e o carte.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Berufe: perechi masculin → feminin (+ -in)', 'der Lehrer → die Lehrerin · der Koch → die Köchin (cu Umlaut!) · der Arzt → die Ärztin.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += fillInBlock('Übung 5 — Posesivele „mein" + „dein" + sinteza Akkusativ', 'mein (m, n): mein Vater · meine (f): meine Mutter. La Akk masculin: mein → meinen (Ich habe meinen Vater).', ex5Data);
        }
        return html;
    }
